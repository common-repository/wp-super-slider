...
class WPSS_WPS_Slide_Renderer {
	...
	public function render() {
		$classes = 'wps-slide';
		$classes = apply_filters( 'wpsus_slide_classes' , $classes, $this->slider_id, $this->slide_index );
		...
		if ( $this->has_thumbnail_content() ) {
			...
			if ( strpos( $thumbnail_content, '[wps_thumbnail_image]' ) !== false ) {
				$thumbnail_content = str_replace( '[wps_thumbnail_image]', $thumbnail_image, $thumbnail_content );
			}

			$classes = "wps-thumbnail";
			$classes = apply_filters( 'wpsus_thumbnail_classes', $classes, $this->slider_id, $this->slide_index );
		...
		if ( $this->has_caption() ) {
			$classes = "wps-caption";
			$classes = apply_filters( 'wpsus_caption_classes', $classes, $this->slider_id, $this->slide_index );
		...
		$this->html_output = apply_filters( 'wpsus_slide_markup', $this->html_output, $this->slider_id, $this->slide_index );
	...
	protected function create_main_image() {
		$main_image_source = $this->lazy_loading === true ? ' src="' . plugins_url( '../public/assets/css/images/blank.gif',__FILE__ ) . '" data-src="' . esc_attr( $this->data['main_image_source'] ) . '"' : ' src="' . esc_attr( $this->data['main_image_source'] ) . '"';
		...
		$classes = apply_filters( 'wpsus_main_image_classes', $classes, $this->slider_id, $this->slide_index );
	...
	protected function add_link_to_main_image( $image ) {
		...
		$main_image_link_href = apply_filters( 'wpsus_slide_link_url', $main_image_link_href, $this->slider_id, $this->slide_index );

		$classes = "";
		$classes = apply_filters( 'wpsus_slide_link_classes', $classes, $this->slider_id, $this->slide_index );
	...
	protected function create_thumbnail_image() {
		...
		$thumbnail_source = $this->lazy_loading === true ? ' src="' . plugins_url( '../public/assets/css/images/blank.gif',__FILE__ ) . '" data-src="' . esc_attr( $thumbnail_source ) . '"' : ' src="' . esc_attr( $thumbnail_source ) . '"';
	...
	protected function add_link_to_thumbnail( $image ) {
		...
		$thumbnail_link_href = apply_filters( 'wpsus_thumbnail_link_url', $thumbnail_link_href, $this->slider_id, $this->slide_index );

		$classes = "";
		$classes = apply_filters( 'wpsus_thumbnail_link_classes', $classes, $this->slider_id, $this->slide_index );
	...
	protected function create_caption() {
		...
		$caption = apply_filters( 'wpsus_slide_caption', $caption, $this->slider_id, $this->slide_index );
	...
	protected function create_html() {
		...
		$html = apply_filters( 'wpsus_slide_html', $html, $this->slider_id, $this->slide_index );
	...
	protected function create_layer( $data ) {
		$layer = WPSS_WPS_Layer_Renderer_Factory::create_layer( $data );
