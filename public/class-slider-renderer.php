 ...
 * @since 1.0.0
 */
class WPSS_WPS_Slider_Renderer {
	...
	public function __construct( $data ) {
		...
		$this->default_settings = WPSS_WpSus_Settings::getSettings();

		$this->idAttribute = isset( $this->settings['use_name_as_id'] ) && $this->settings['use_name_as_id'] === true ? str_replace( ' ', '-', strtolower( $this->name ) ) : 'wp-sus-' . $this->id;
	}
	...
	public function render() {
		...
		$this->html_output = apply_filters( 'wpsus_markup', $this->html_output, $this->id );
	...
	protected function create_slide( $data, $slide_counter ) {
		...
		$slide = WPSS_WPS_Slide_Renderer_Factory::create_slide( $data );
	...
	public function render_js() {
		...
		if ( isset ( $this->settings['lightbox'] ) && $this->settings['lightbox'] === true ) {
			...
			$lightbox_options = apply_filters( 'wpsus_lightbox_options', $lightbox_options, $this->id );
