...
class WPSS_WpSus_Admin {
	...
	/**
	 * Current class instance of the public wp sus class.
	 * 
	 * @since 1.0.0
	 * 
	 * @var object
	...
	private function __construct() {
		$this->plugin = WPSS_WpSus::get_instance();
		...
		add_action( 'wp_ajax_wpsus_get_slider_data', array( $this, 'ajax_get_slider_data' ) );
		add_action( 'wp_ajax_wpsus_save_slider', array( $this, 'ajax_save_slider' ) );
		add_action( 'wp_ajax_wpsus_preview_slider', array( $this, 'ajax_preview_slider' ) );
		add_action( 'wp_ajax_wpsus_update_presets', array( $this, 'ajax_update_presets' ) );
		add_action( 'wp_ajax_wpsus_get_preset_settings', array( $this, 'ajax_get_preset_settings' ) );
		add_action( 'wp_ajax_wpsus_get_breakpoints_preset', array( $this, 'ajax_get_breakpoints_preset' ) );
		add_action( 'wp_ajax_wpsus_delete_slider', array( $this, 'ajax_delete_slider' ) );
		add_action( 'wp_ajax_wpsus_duplicate_slider', array( $this, 'ajax_duplicate_slider' ) );
		add_action( 'wp_ajax_wpsus_export_slider', array( $this, 'ajax_export_slider' ) );
		add_action( 'wp_ajax_wpsus_import_slider', array( $this, 'ajax_import_slider' ) );
		add_action( 'wp_ajax_wpsus_add_slides', array( $this, 'ajax_add_slides' ) );
		add_action( 'wp_ajax_wpsus_load_main_image_editor', array( $this, 'ajax_load_main_image_editor' ) );
		add_action( 'wp_ajax_wpsus_load_thumbnail_editor', array( $this, 'ajax_load_thumbnail_editor' ) );
		add_action( 'wp_ajax_wpsus_load_caption_editor', array( $this, 'ajax_load_caption_editor' ) );
		add_action( 'wp_ajax_wpsus_load_html_editor', array( $this, 'ajax_load_html_editor' ) );
		add_action( 'wp_ajax_wpsus_load_layers_editor', array( $this, 'ajax_load_layers_editor' ) );
		add_action( 'wp_ajax_wpsus_add_layer_settings', array( $this, 'ajax_add_layer_settings' ) );
		add_action( 'wp_ajax_wpsus_load_settings_editor', array( $this, 'ajax_load_settings_editor' ) );
		add_action( 'wp_ajax_wpsus_load_content_type_settings', array( $this, 'ajax_load_content_type_settings' ) );
		add_action( 'wp_ajax_wpsus_add_breakpoint', array( $this, 'ajax_add_breakpoint' ) );
		add_action( 'wp_ajax_wpsus_add_breakpoint_setting', array( $this, 'ajax_add_breakpoint_setting' ) );
		add_action( 'wp_ajax_wpsus_get_taxonomies', array( $this, 'ajax_get_taxonomies' ) );
		add_action( 'wp_ajax_wpsus_clear_all_cache', array( $this, 'ajax_clear_all_cache' ) );
		add_action( 'wp_ajax_wpsus_close_getting_started', array( $this, 'ajax_close_getting_started' ) );
	}
		...
		if ( in_array( $screen->id, $this->plugin_screen_hook_suffixes ) ) {
			...
			if ( get_option( 'wpsus_is_custom_css') == true ) {
				if ( get_option( 'wpsus_load_custom_css_js' ) === 'in_files' ) {
					...
					$custom_css_path = plugins_url( 'wpsus-custom/custom' . $file_suffix . '.css' );
					$custom_css_dir_path = WP_PLUGIN_DIR . '/wpsus-custom/custom' . $file_suffix . '.css';

					if ( file_exists( $custom_css_dir_path ) ) {
						wp_enqueue_style( $this->plugin_slug . '-plugin-custom-style', $custom_css_path, array(), WPSS_WpSus::VERSION );
					}
				} else {
	...
	public function enqueue_admin_scripts() {
		...
		if ( in_array( $screen->id, $this->plugin_screen_hook_suffixes ) ) {
			...
			if ( get_option( 'wpsus_is_custom_js' ) == true && get_option( 'wpsus_load_custom_css_js' ) === 'in_files' ) {
				...
				$custom_js_path = plugins_url( 'wpsus-custom/custom' . $file_suffix . '.js' );
				$custom_js_dir_path = WP_PLUGIN_DIR . '/wpsus-custom/custom' . $file_suffix . '.js';

				if ( file_exists( $custom_js_dir_path ) ) {
					wp_enqueue_script( $this->plugin_slug . '-plugin-custom-script', $custom_js_path, array(), WPSS_WpSus::VERSION );
				}
			}

			$id = isset( $_GET['id'] ) ? $_GET['id'] : -1;

			wp_localize_script( $this->plugin_slug . '-admin-script', 'wps_js_vars', array(
				...
				'slider_delete' => __( 'Are you sure you want to delete this slider?', 'wpsus' ),
				'slide_delete' => __( 'Are you sure you want to delete this slide?', 'wpsus' ),
				'preset_name' => __( 'Preset Name:', 'wpsus' ),
				'preset_update' => __( 'Are you sure you want to override the selected preset?', 'wpsus' ),
				'preset_delete' => __( 'Are you sure you want to delete the selected preset?', 'wpsus' ),
				'yes' => __( 'Yes', 'wpsus' ),
				'cancel' => __( 'Cancel', 'wpsus' ),
				'save' => __( 'Save', 'wpsus' ),
				'slider_update' => __( 'Slider updated.', 'wpsus' ),
				'slider_create' => __( 'Slider created.', 'wpsus' )
			) );
		}
	}
	...
	public function add_admin_menu() {
		...
		$access = get_option( 'wpsus_access', $plugin_settings['access']['default_value'] );

		$restricted_pages = array();
		$restricted_pages = apply_filters( 'wpsus_restricted_pages' , $restricted_pages );
	...
	public function render_custom_css_js_page() {
		$custom_css = get_option( 'wpsus_custom_css', '' );
		$custom_js = get_option( 'wpsus_custom_js', '' );
		...
		if ( isset( $_POST['custom_css_update'] ) || isset( $_POST['custom_js_update'] ) ) {
			...
			if ( isset( $_POST['custom_css'] ) ) {
				...
				if ( $custom_css !== '' ) {
					update_option( 'wpsus_is_custom_css', true );
				} else {
					update_option( 'wpsus_is_custom_css', false );
				}
			}
			...
			if ( isset( $_POST['custom_js'] ) ) {
				...
				if ( $custom_js !== '' ) {
					update_option( 'wpsus_is_custom_js', true );
				} else {
					update_option( 'wpsus_is_custom_js', false );
				}
			}
	 ...
	 * it in the options table.
	 *
	 * It verifies the activatewp code supplied and displays
	 * if it's valid.
	 * 
	 * @since 1.0.0
	 */
	public function render_plugin_settings_page() {
		$plugin_settings = WPSS_WpSus_Settings::getPluginSettings();
		$load_stylesheets = get_option( 'wpsus_load_stylesheets', $plugin_settings['load_stylesheets']['default_value'] );
		$load_custom_css_js = get_option( 'wpsus_load_custom_css_js', $plugin_settings['load_custom_css_js']['default_value'] );
		$load_js_in_all_pages = get_option( 'wpsus_load_js_in_all_pages', $plugin_settings['load_js_in_all_pages']['default_value'] );
		$load_unminified_scripts = get_option( 'wpsus_load_unminified_scripts', $plugin_settings['load_unminified_scripts']['default_value'] );
		$cache_expiry_interval = get_option( 'wpsus_cache_expiry_interval', $plugin_settings['cache_expiry_interval']['default_value'] );
		$hide_inline_info = get_option( 'wpsus_hide_inline_info', $plugin_settings['hide_inline_info']['default_value'] );
		$hide_getting_started_info = get_option( 'wpsus_hide_getting_started_info', $plugin_settings['hide_getting_started_info']['default_value'] );
		$access = get_option( 'wpsus_access', $plugin_settings['access']['default_value'] );
		...
		if ( isset( $_POST['plugin_settings_update'] ) ) {
			...
			if ( isset( $_POST['load_js_in_all_pages'] ) ) {
				...
				update_option( 'wpsus_load_js_in_all_pages', true );
			} else {
				$load_js_in_all_pages = false;
				update_option( 'wpsus_load_js_in_all_pages', false );
			}

			if ( isset( $_POST['load_unminified_scripts'] ) ) {
				$load_unminified_scripts = true;
				update_option( 'wpsus_load_unminified_scripts', true );
			} else {
				$load_unminified_scripts = false;
				update_option( 'wpsus_load_unminified_scripts', false );
			}
			...
			if ( isset( $_POST['hide_inline_info'] ) ) {
				...
				update_option( 'wpsus_hide_inline_info', true );
			} else {
				$hide_inline_info = false;
				update_option( 'wpsus_hide_inline_info', false );
			}

			if ( isset( $_POST['hide_getting_started_info'] ) ) {
				$hide_getting_started_info = true;
				update_option( 'wpsus_hide_getting_started_info', true );
			} else {
				$hide_getting_started_info = false;
				update_option( 'wpsus_hide_getting_started_info', false );
			}
		...
		$activatewp_pluginok = get_option( 'wpsus_activatewp_pluginok', '' );
		$activatewp_pluginok_status = get_option( 'wpsus_activatewp_pluginok_status', '0' );
		
		if ( isset( $_POST['activatewp_pluginok_update'] ) ) {
			...
			if ( isset( $_POST['activatewp_pluginok'] ) ) {
				...
				if ( $_POST['activatewp_pluginok'] === '' ) {
					$activatewp_pluginok_status = '0';
				} else {
					$api = WPSS_WpSus_API::get_instance();

					$verification_result = $api->verify_activatewp_pluginok( $activatewp_pluginok );

					if ( $verification_result === 'yes' ) {
						$activatewp_pluginok_status = '1';
					} else if ( $verification_result === 'no' ) {
						$activatewp_pluginok_status = '2';
					} else if ( $verification_result === 'error' ) {
						$activatewp_pluginok_status = '3';
					}
				}

				update_option( 'wpsus_activatewp_pluginok_status', $activatewp_pluginok_status );
			}
		}
	...
	private function save_custom_css_js_in_files ( $custom_css, $custom_js ) {
		$url = wp_nonce_url( 'admin.php?page=wpsus-custom', 'custom-css-js-update', 'custom-css-js-nonce' );
		...
		// create the 'wpsus-custom' folder if it doesn't exist
		if ( ! $wp_filesystem->exists( $context . '/wpsus-custom' ) ) {
			$wp_filesystem->mkdir( $context . '/wpsus-custom' );
		}
		...
		$wp_filesystem->put_contents( $context . '/wpsus-custom/custom' . $file_suffix . '.css', stripslashes( $custom_css ), FS_CHMOD_FILE );
		$wp_filesystem->put_contents( $context . '/wpsus-custom/custom' . $file_suffix . '.js', stripslashes( $custom_js ), FS_CHMOD_FILE );
	}
	...
	public function save_slider( $slider_data ) {
		...
		if ( $id === -1 ) {
			$wpdb->insert($wpdb->prefix . 'wp_sus_sliders', array( 'name' => $slider_data['name'],
			...
			$wpdb->update( $wpdb->prefix . 'wp_sus_sliders', array( 'name' => $slider_data['name'], 
																	 	'settings' => json_encode( $slider_data['settings'] ),
																	 	'modified' => date( 'm-d-Y' ),
																		'panels_state' => json_encode( $slider_data['panels_state'] ) ), 
																	   	array( 'id' => $id ), 
			...
			$wpdb->query( $wpdb->prepare( "DELETE FROM " . $wpdb->prefix . "wp_sus_slides WHERE slider_id = %d", $id ) );
		...
		foreach ( $slides_data as $slide_data ) {
			...
			$wpdb->insert( $wpdb->prefix . 'wp_sus_slides', $slide, array( '%d', '%s', '%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' ) );
			...
			if ( ! empty( $slide_data[ 'layers' ] ) ) {
				...
				foreach ( $layers_data as $layer_data ) {
					...
					$wpdb->insert( $wpdb->prefix . 'wp_sus_layers', $layer, array( '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' ) );
				}
			}
		}
		
		delete_transient( 'wpsus_cache_' . $id );
	...
	public function ajax_update_presets() {
		...
		update_option( 'wpsus_setting_presets', $presets );
	...
	public function delete_slider( $id ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM " . $wpdb->prefix . "wp_sus_sliders WHERE id = %d", $id ) );
	...
	public function get_post_names() {
		$result = array();
		$post_names_transient = get_transient( 'wpsus_post_names' );
		...
		if ( $post_names_transient === false ) {
			...
			set_transient( 'wpsus_post_names', $result, 5 * 60 );
		} else {
	 ...
	 * stored in the 'wpsus_posts_data' transient. If there is any
	...
	public function get_taxonomies_for_posts( $post_names ) {
		...
		$posts_data_transient = get_transient( 'wpsus_posts_data' );
		...
		set_transient( 'wpsus_posts_data', $posts_data_transient, 5 * 60 );
	...
	public function create_breakpoint_setting( $name, $value ) {
		$setting = WPSS_WpSus_Settings::getSettings( $name );
	...
	public function ajax_clear_all_cache() {
		...
		$wpdb->query( "DELETE FROM " . $wpdb->prefix . "options WHERE option_name LIKE '%wpsus_cache%'" );
